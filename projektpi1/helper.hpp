...
//size, pos, color, hovColor, texture, (optional): text, font, charSize
class Button {
public:
...
public:
    ...
    void draw(sf::RenderWindow& window, sf::Vector2f position, float rotation, float length, float thickness) {
        ...
        bar.setSize({80, 22});
        HSL colorHSL(120 - (int)(120 * (length / 80)), 0.5f, 0.5f);
        RGB color = HSLToRGB(colorHSL);
        bar.setFillColor(sf::Color(color.R, color.G, color.B));
        sf::IntRect a({ (int)length - 81, 1 }, { 82, 24 });
        bar.setTextureRect(a);
